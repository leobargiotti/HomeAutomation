...
EVENT_RESOURCE(humidity_resource,
         "title=\"humidity\";rt=\"Text\"", //DA SISTEMARE
         NULL,
         NULL,
         res_put_handler,
         NULL,
...
static void res_put_handler(coap_message_t *request, coap_message_t *response, uint8_t *buffer, uint16_t preferred_size, int32_t *offset){
    ...
    if(threshold!=NULL && strlen(threshold)!=0) {
        ...
        else{ //the humidity is now in the optimal interval
            if(action!=NULL && strlen(action)>0){
                if(strcmp(action, "ON_UP")==0 && !actuator_on){ //the actuator is off and the user wants to turn it on to humidify
                    //coap, led and log
                ...
                else if(strcmp(action, "ON_DOWN")==0 && !actuator_on){ //the actuator is off and the user wants to turn it on to dry
                    //coap, led and log
                    LOG_INFO("dehumidifier on for user input\n");
                    coap_set_status_code(response,CHANGED_2_04);
                    leds_on(LEDS_GREEN);
